import { ArrowLeft, LockKeyhole, ShieldAlert, ShieldCheck, UtensilsCrossed } from 'lucide-react'
...
export default function AccessDeniedPage() {
  ...
  const security = auth.user?.role === 'SECURITY'
  const serviceName = service === 'events' ? 'Event Access' : service === 'canteen' ? 'Canteen Orders' : service === 'security' ? 'Security Desk' : service === 'admin' ? 'administrator tools' : 'Space Bookings'
  ...
  return <main className="grid min-h-screen place-items-center bg-[#f7f6f3] px-5 py-12 text-[#191919] dark:bg-[#191919] dark:text-[#f7f6f3]">
    <section className="w-full max-w-xl">
      ...
      <div className="rounded-3xl border-2 border-black bg-white p-8 shadow-[8px_8px_0_#191919] dark:border-white dark:bg-neutral-900 dark:shadow-[8px_8px_0_#f7f6f3]">
        ...
        <div className="mt-7 flex flex-wrap gap-3"><Link className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white hover:bg-blue-700" to={canteenStaff ? '/app/canteen/manage' : security ? '/app/security' : '/app'}>{canteenStaff ? <UtensilsCrossed size={18} /> : security ? <ShieldCheck size={18} /> : <ArrowLeft size={18} />}{canteenStaff ? 'Return to Canteen Console' : security ? 'Return to Security Desk' : 'Return to Space Bookings'}</Link><Link className="rounded-xl border-2 border-black px-5 py-3 font-semibold dark:border-white" to="/">View all products</Link></div>
      </div>
    </section>
  </main>
}
